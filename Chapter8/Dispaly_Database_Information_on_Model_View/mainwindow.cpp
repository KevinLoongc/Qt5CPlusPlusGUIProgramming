    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->tableWidget->setColumnHidden(0, true);  // Hide the  ID column
    ...
    if (db.open())
    {
        ...
        if (query.exec("SELECT id, name, age, gender, married FROM employee"))
        {
            while (query.next())
            {
                ...
                ui->tableWidget->setRowCount(ui->tableWidget->rowCount() + 1);  // Next row
...
void MainWindow::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    if (hasInit)
    {
        ...
        {
            ui->tableWidget->item(item->row(), 3)->setData(0, "Feamle");
            gender = "1";
        }
        ...
        {
            QMessageBox::information(this, "Update Success", "Date update to database.");
        }
        else
        {
            qDebug() << query.lastError().text();
